...
#define DOCTEST_VERSION_STR "2.0.1-LiU"
...
// use -Wno-format to suppress format checks of gcc
// (errors are caused by mingw gcc (using msvc runtime))
DOCTEST_GCC_SUPPRESS_WARNING("-Wformat")
DOCTEST_GCC_SUPPRESS_WARNING("-Wformat-extra-args")
#endif
// end LiU changes
...
// LiU changes: need to load cstdio while strict ansi is disabled (otherwise fileno is not defined)
...
#define DOCTEST_FAST_ASSERT_IN_TESTS(decomp)                                     \
    ...
    if (rb.m_failed && checkIfShouldThrow(at)) res |= assertAction::shouldthrow; \
    DOCTEST_FAST_ASSERT_REACT

template <int comparison, typename L, typename R>
DOCTEST_NOINLINE DOCTEST_FAST_ASSERT_RETURN_TYPE fast_binary_assert(assertType::Enum at,
...
int Context::run() {
...
#ifdef DOCTEST_PLATFORM_WINDOWS
    if (isDebuggerActive()) p->reporters_currently_used.push_back(&g_debug_output_rep);
#endif  // DOCTEST_PLATFORM_WINDOWS

    // handle version, help and no_run
    if (p->no_run || p->version || p->help || p->list_reporters) {
        if (p->version) g_con_rep.printVersion();
        if (p->help) g_con_rep.printHelp();
        if (p->list_reporters) g_con_rep.printRegisteredReporters();
    ...
    if (p->list_test_cases) g_con_rep.output_query_preamble_test_cases();

    std::set<String> testSuitesPassingFilt;
    if (p->list_test_suites) g_con_rep.output_query_preamble_test_suites();
